...
//@reply to question
exports.replyQuestion = (req, res, next) => {
  var { orgCode } = req.body;
  organisation
    .findOne({ orgCode })
    .then((orgFound) => {
...
//@read all questions
exports.readQuestion = (req, res, next) => {
...
//@read reply to a question
exports.readReply = (req, res, next) => {
    ...
    .then((orgFound) => {
      if (orgFound) {
        ...
        if (questionIndex >= 0 && orgFound.questionaire[questionIndex].active) {
          ...
          const questionList = orgFound.questionaire[questionIndex].replies.map(
            (questionReplySingle) => {
  ...
  organisation
    ...
    .then((orgFound) => {
      if (orgFound) {
        ...
        if (questionIndex >= 0 && orgFound.questionaire[questionIndex].active) {
          orgFound.questionaire[questionIndex].active = !orgFound.questionaire[
            questionIndex
          ].active;
          orgFound
            .save()
  ...
  organisation
    ...
    .then((orgFound) => {
      if (orgFound) {
        ...
        if (questionIndex >= 0 && orgFound.questionaire[questionIndex].active) {
          var replyIndex = _.findIndex(
            orgFound.questionaire[questionIndex].replies,
            {
              id: replyId,
            }
          );
          if (
            replyIndex >= 0 &&
            orgFound.questionaire[questionIndex].replies[replyIndex].active
          ) {
            orgFound.questionaire[questionIndex].replies[
              replyIndex
            ].active = !orgFound.questionaire[questionIndex].replies[replyIndex]
              .active;
            orgFound
              .save()
              .then((replyDeleted) => {
